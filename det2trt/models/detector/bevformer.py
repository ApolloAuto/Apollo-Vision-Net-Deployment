...
class BEVFormerTRT(BEVFormer):
    ...
    def forward_trt_occ_det(self, image, prev_bev, use_prev_bev, can_bus, lidar2img, no_pad_image_shape):
        img_feats = self.extract_feat(img=image)
        assert len(img_feats[0]) == 1
        bev_embed, outputs_classes, outputs_coords, outputs_occupancy = self.pts_bbox_head.forward_trt(
            img_feats, prev_bev, can_bus, lidar2img, no_pad_image_shape, use_prev_bev
        )
        outputs_classes = outputs_classes.sigmoid()
        outputs_occupancy = outputs_occupancy.sigmoid()
